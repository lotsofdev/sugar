...
export default function __hotkey(key, callback, settings) {
    ...
    const isMultipleSpecialKeys = keys.filter((k) => specialKeys.includes(k)).length > 1;
    const _handler = (e) => {
        ...
        let multipleSpecialKeys = [e.shiftKey, e.ctrlKey, e.altKey, e.metaKey].filter((k) => k).length >
            1, hasSpecialKeys = e.shiftKey || e.ctrlKey || e.altKey || e.metaKey;
